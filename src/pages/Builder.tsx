...
import { createMountStructure, runPreviewSetup } from '../lib/webcontainerSetup';
...
export function Builder() {
  ...
  const { webcontainer, previewUrl, previewError } = useWebContainer();
  const previewSetupRef = useRef<string>('');
  const previewSetupGeneration = useRef(0);
  ...
  const [isPreviewSettingUp, setIsPreviewSettingUp] = useState(false);
  ...
  useEffect(() => {
    if (!webcontainer || files.length === 0 || loading) return;

    const fingerprint = JSON.stringify(
      files.map((f) => ({ path: f.path, content: f.content }))
    );
    if (previewSetupRef.current === fingerprint) return;
    previewSetupRef.current = fingerprint;

    const generation = ++previewSetupGeneration.current;

    (async () => {
      setIsPreviewSettingUp(true);
      try {
        const mountStructure = createMountStructure(files);
        await webcontainer.mount(mountStructure);
        if (generation !== previewSetupGeneration.current) return;
        await runPreviewSetup(webcontainer);
      } catch (error) {
        console.error('❌ Mount or preview setup failed:', error);
      } finally {
        if (generation === previewSetupGeneration.current) {
          setIsPreviewSettingUp(false);
        }
      }
    })();
  }, [files, webcontainer, loading]);
                ...
                >
                  {showInteractiveLoader ? (
                    ...
                    <PreviewFrame
                      previewUrl={previewUrl}
                      previewError={previewError}
                      isSettingUp={isPreviewSettingUp}
                    />
                  )}
                </motion.div>
              )}
