...
export function InteractiveLoader({ onComplete, files, previewUrl, previewError }: InteractiveLoaderProps) {
  ...
  }, [previewUrl, previewError, onComplete]);

  useEffect(() => {
    if (visibleBulletins.length !== bulletins.length) return;
    const fallback = setTimeout(onComplete, 120000);
